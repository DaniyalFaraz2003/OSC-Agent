...
export class ContextBuilder {
  ...
  static build(issue: string, analysis: string, results: CodeSearchResult[]): string {
    ...
    if (results.length === 0) {
      fileContext =
        '(No existing source files were found for this issue.\n' +
      ...
      const sourceFiles = results.filter(
        (r) => !r.filePath.endsWith('package.json') && !r.filePath.endsWith('tsconfig.json'),
      );
      const configFiles = results.filter(
        (r) => r.filePath.endsWith('package.json') || r.filePath.endsWith('tsconfig.json'),
      );
      ...
      if (sourceFiles.length > 0) {
        ...
        parts.push(
          '(No relevant source files were found. You will likely need to CREATE new files.\n' +
