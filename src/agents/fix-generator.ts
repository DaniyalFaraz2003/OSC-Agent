...
export class FixGenerator {
  ...
  async generateFix(issueDescription: string, analysis: string, searchResults: CodeSearchResult[], strategy: FixStrategy = 'minimal'): Promise<FixProposal> {
    ...
    // Just use the raw content as explanation
    const explanation = this.cleanResponse(response.content);

    return {
      explanation,
      confidenceScore: 0.95, // fixed default for now
      patches: [], // no patches in explanation-only mode
      strategy,
      usage: response.usage,
    };
  }
